...
(function () {
  ...
  function FlowApp() {
    ...
    const hasIdentity = !!(uc && uc.selectedCollege && uc.selectedCollege.name && uc.selectedMajor && uc.selectedMajor.name);
    const identityKey = FlowStore.identityKeyFromUserContext(uc);
    const saved = FlowStore.loadForIdentity(identityKey);
    React.useEffect(() => {
    ...
    useEffect(() => { FlowStore.save({ identityKey, phase, ctx, sectionIdx, answers }); }, [identityKey, phase, ctx, sectionIdx, answers]);
    ...
    return <Report report={report}
      ...
      onRestart={() => { FlowStore.wipe(); if (window.UserContext) window.UserContext.clear(); setAnswers({}); setSectionIdx(0); setCtx(emptyCtx); toStart(); }}
    />;
  }
