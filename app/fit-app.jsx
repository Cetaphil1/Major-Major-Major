...
(function () {
  ...
  function FlowApp() {
    ...
    const activeIdentityKey = FlowState.identityKeyFromUserContext(uc);
    const saved = FlowState.load(activeIdentityKey);

    React.useEffect(() => {
      if (!uc || !uc.preLandingComplete || !activeIdentityKey) { window.location.replace("start.html"); }
    }, []);
    ...
    const flowIdentityKey = activeIdentityKey || FlowState.identityKeyFromFlowContext(ctx);
    useEffect(() => {
      FlowState.save({ phase, ctx, sectionIdx, answers }, flowIdentityKey);
