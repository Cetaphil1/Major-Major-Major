...
(function () {
  ...
  function load(uc) { return FlowState.loadForUserContext(uc); }
  function save(s, uc) { return FlowState.saveForUserContext(s, uc); }
  ...
  function FlowApp() {
    ...
    const saved = hasIdentity(uc) ? load(uc) : null;
    React.useEffect(() => {
      if (!hasIdentity(uc)) { window.location.replace("start.html"); }
    }, []);
    ...
    useEffect(() => {
      if (hasIdentity(uc)) save({ phase, ctx, sectionIdx, answers }, uc);
