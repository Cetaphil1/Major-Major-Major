...
(function () {
  ...
  function identityFor(uc) { return FlowState.identityFromUserContext(uc); }
  function load(identity) { return FlowState.load(identity); }
  function save(s, identity) { return FlowState.save(s, identity); }
  ...
  function FlowApp() {
    ...
    const identity = identityFor(uc);
    const saved = load(identity);
    React.useEffect(() => {
    ...
    useEffect(() => { save({ phase, ctx, sectionIdx, answers }, identity); }, [phase, ctx, sectionIdx, answers]);
